...
var GoogleAdwords = function (spec, my) {
  ...
  my.auth = {}
  my.agent = spec.agent
  my.host = spec.host || 'https://adwords.google.com/api/adwords/reportdownload/v201609'
  ...
  var _sendAWQL = function () {
    return new BBPromise(function (resolve, reject) {
      ...
      if (builtAWQL.indexOf(",,") > -1) {
        builtAWQL = builtAWQL.replace(/\,\,/g, ",");
      }
  ...
  var _buildHeader = function (bodyLength) {
    ...
    var includeZeroImpressions = true;
    if (self.includeZeroImpressions === false)
  ...
  self.awql = function (options) {
    if (options) {
      self.includeZeroImpressions = options.includeZeroImpressions;
